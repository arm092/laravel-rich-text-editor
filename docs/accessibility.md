...
The Table dropdown supports keyboard entry, closes with Escape, and is constrained to the mobile viewport. Tab and Shift+Tab navigate table cells, selected cells have a visible state, and responsive table containers provide horizontal scrolling without causing page overflow. The table resize handle exposes slider semantics: arrow keys resize by 5%, Home selects 20%, and End restores full width.
